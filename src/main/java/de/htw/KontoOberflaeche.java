...
public class KontoOberflaeche extends BorderPane {
	 ...
	 */
	public KontoOberflaeche(Girokonto girokonto, App app){
		this.app = app;
		this.giro = girokonto;
		...
		txtNummer = new Text("Kontonummer: " + giro.getKontonummer());
		...
		stand.textProperty().bind(giro.kontostandProperty().asString());
		stand.fillProperty().bind(Bindings.when(giro.negativeProperty()).then(Color.RED).otherwise(Color.GREEN));
		txtGesperrt = new Text("Gesperrt: ");
		txtGesperrt.setFont(new Font("Sans Serif", 15));
		anzeige.add(txtGesperrt, 0, 2);
		gesperrt = new CheckBox();
		gesperrt.selectedProperty().bindBidirectional(this.giro.gesperrtProperty());
		...
		einzahlen.setOnAction((ActionEvent e) -> {
			try {
				app.einzahlen(Double.parseDouble(betrag.getText()));
			}
			catch (Exception ignored){
				Alert alert = new Alert(Alert.AlertType.WARNING);
				alert.setContentText("Beim Einzahlen ist ein Fehler aufgeteten.");
				alert.show();
			}
				});
		abheben.setOnAction((ActionEvent e) -> {
			try {
				app.abheben(Double.parseDouble(betrag.getText()));
			}catch (Exception ignored){
				Alert alert = new Alert(Alert.AlertType.WARNING);
				alert.setContentText("Beim Abheben ist ein Fehler aufgetreten.");
				alert.show();
			}
